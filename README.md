...
We leverage a TGNN model, either `TGAT`, `CAWN`, `TCL`, `GraphMixer` or `DyGFormer`, as the dynamic backbone. It does the job of integrating temporal information (timestamps) and graph structure information, along with feature vectors of edges and/or nodes, into node embeddings.

We composes our predictor model, which uses these node embeddings to do link prediction, edge classification and node classification tasks.

The link prediction is a pretraining task and we do not report its ROC_AUC score. (In fact, we do not even need to evaluate its ROC_AUC score if we use loss for early stopping. There is not any evaluation phase.) The predictor is similarity-based and prompt-based. Every training batches are composed of two equally-sized groups of directed edges. The two groups also have exactly the same source nodes of edges. In the positive group, the destionation nodes are connected to each corresponding source node; while in the negative group they are not. A generic task prompt is first multiplied to the node embeddings, and the similarity of each (source node, destination node) pair is calculated and compared to binary value of whether there exists such an directed edge between the two nodes. The similarity can be implemented as consine-similarity, but we use MLP for better model learning capability. The parameters of the MLP will be fixed in classification tasks if pretraining-prompt architechure is employed, learnable if pretraining-finetuning architecture.

Edge classification predictor has three options: the `learnable` classifier, the `mean` classifier and the `mlp` classifier. The `mlp` classifier use pretraining-finetuning architecture, and the class predicted is mlp(source node embedding, destination node embedding). The `learnable` and `mean` classifiers use pretraing-prompt architecture: When determining the class for an edge, we compare it to each prototypical edge that is considered to be the center edges for each class. By "compare", we mean the similarity is calculated. (Again, cosine similarity is more intuitive, but similarity calculated by MLP has superior performance.) The edge is classifed to the class whose prototypical edges is most similar to the edge itself. In the `learnable` classifier, the prototypical edges are learnable parameter tensors randomly initialized. In the `mean` setting, the prototypical edges are the mean values of all currunt node embeddings of the same classes. The `mean` prototypical edges need to be calculated over all training data for every epoch (since the node embedding changes during training), so training time bececame $O(n^2)$ from $O(n)$, where $n$ is the number of training data. However, it is still computable since we are dealing with few-shot learning problem here.

Node classification task is converted to edge classification. We add a self-loop for the node to be classified and classify the self-loop instead.
