...
def get_link_prediction_args():
    ...
    parser.add_argument('--dataset_name', type=str, help='dataset to be used', default='wikipedia',
                        choices=['wikipedia', 'moocact', 'bitcoinalpha', 'bitcoinotc', 'hyperlink', 'reddit', 'mooc', 'lastfm', 'enron', 'SocialEvo', 'uci', 'Flights', 'CanParl', 'USLegis', 'UNtrade', 'UNvote', 'Contacts'])
...
def load_link_prediction_best_configs(args: argparse.Namespace):
    ...
    if args.model_name == 'TGAT':
        ...
        elif args.dataset_name in ['moocact', 'bitcoinotc', 'hyperlink', 'reddit', 'CanParl', 'UNtrade']:
            args.sample_neighbor_strategy = 'uniform'
        else:
...
def get_node_classification_args():
    ...
    parser.add_argument('--test_interval_epochs', type=int, default=10, help='how many epochs to perform testing once')
    parser.add_argument('--load_best_configs', action='store_true', default=False, help='whether to load the best configurations')
    parser.add_argument('--baseline', action='store_true', default=False, help='edge classification baseline with pre-training')
...
def load_node_classification_best_configs(args: argparse.Namespace):
    ...
    if args.model_name == 'TGAT':
        ...
        args.num_layers = 2
        args.dropout = 0.1
        if args.dataset_name in ['moocact', 'bitcoinalpha', 'bitcoinotc', 'hyperlink', 'reddit']:
            args.sample_neighbor_strategy = 'uniform'
        else:
            args.sample_neighbor_strategy = 'recent'
    elif args.model_name in ['JODIE', 'DyRep', 'TGN']:
