...
class NegativeEdgeSampler(object):
    ...
    def __init__(self, dst_node_ids: np.ndarray, seed: int = None):
        self.seed = seed
    ...
    def sample(self, batch_src_node_ids):
        return self.random_sample(batch_src_node_ids)

    def random_sample(self, batch_src_node_ids):
        if self.seed is None:
            random_sample_edge_dst_node_indices = np.random.randint(0, len(self.unique_dst_node_ids), len(batch_src_node_ids))
        else:
            random_sample_edge_dst_node_indices = self.random_state.randint(0, len(self.unique_dst_node_ids), len(batch_src_node_ids))
